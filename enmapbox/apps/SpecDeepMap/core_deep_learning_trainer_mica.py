...
class MyModel(L.LightningModule):
    ...
    def configure_optimizers(self):
        ...
        sch = torch.optim.lr_scheduler.StepLR(
            opt,
            step_size=self.n_epochs // 3,  # Reduce at 1/3 and 2/3 of training
            gamma=0.5
        )
        return [opt], [sch]
    ...
    def _unfreeze_first_encoder_layer(self, encoder):
        """
        Unfreeze the first encoder layer for any AMP backbone (conv or transformer).
        This method works generically by finding the first layer with parameters in the encoder.
        For nested structures (like ConvNeXt), it finds the first actual convolutional layer.
        """

        def find_first_conv_layer(module, path=""):
            """Recursively find the first layer with parameters, preferring conv layers."""
            # Check if current module has parameters
            if list(module.parameters()):
                # If this is a conv layer, prefer it
                if 'conv' in module.__class__.__name__.lower() or 'conv2d' in str(type(module)).lower():
                    return module, path
                # Otherwise, continue looking for a conv layer

            # If has children, look deeper
            if hasattr(module, 'named_children'):
                for name, child in module.named_children():
                    child_path = f"{path}.{name}" if path else name
                    result_module, result_path = find_first_conv_layer(child, child_path)
                    if result_module is not None:
                        return result_module, result_path

            # If no conv layer found but current module has parameters, use it
            if list(module.parameters()):
                return module, path

            return None, ""

        # Get the first top-level module
        first_layer = None
        ...
        if first_layer is not None:
            # Find the first conv layer (or first layer with parameters)
            conv_layer, conv_path = find_first_conv_layer(first_layer, first_layer_name)

            if conv_layer is not None:
                # Unfreeze all parameters in the first conv layer
                for param in conv_layer.parameters():
                    param.requires_grad = True
            else:
                assert False, "Could not find first layer to unfreeze"
        else:
            assert False, "Could not find first layer to unfreeze"
    ...
    def _prepare_model(self):
        ...
        if self.freeze_encoder == True:
            ...
            # Always unfreeze the first encoder layer for fine-tuning
            # This works for all AMP backbones (conv or transformer encoders)
            #self._unfreeze_first_encoder_layer(model.encoder)
...
def dl_train(
    ...
    if pretrained_weights == 'Sentinel_2_TOA_Resnet18':
        ...
        backbone = 'resnet50'
    elif pretrained_weights == 'MicaSense_SR_Swin_s3_tiny':
